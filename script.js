...
(function () {
  ...
  /* ═══════════════════════════════════════════
      Greeting Section (★ 수정된 구역)
      ═══════════════════════════════════════════ */

 // script.js의 initGreeting() 함수 내부를 아래 코드로 대체하세요.
...
// 부모님 성함 분기 처리용 헬퍼 함수
  function renderParentLine(p, relation) {
    ...
    if (p.father && p.mother) {
      ...
      // 신부측처럼 한 분만 계시는 경우 점(·)을 빼고 깔끔하게 출력
  ...
  $('#greetingParents').innerHTML = parentsHTML;
    ...
    // ─── [추가] 인사말 밑에 전화/문자 링크 연동하기 ───
    // 만약 데이터 관리용 파일(config.js 등)에 groom.tel 이나 bride.tel을 추가하셨다면 자동으로 연동되며,
    // 없을 경우 아래 적어둔 기본 번호(예시)로 작동합니다. 아래 번호를 직접 고치셔도 무방합니다.
    const groomPhone = g.tel || '01012345678'; 
    const bridePhone = b.tel || '01087654321';

    const contactHTML = `
      <div class="contact-list animate-item" data-animate="fade-up">
          <div class="contact-item">
              <div class="contact-item__info">
                  <span class="contact-item__role">신랑</span>
                  <div class="contact-item__name">${g.name}</div>
              </div>
              <div class="contact-item__actions">
                  <a href="tel:${groomPhone}" class="btn btn--outline btn--contact" title="신랑에게 전화하기">
                      <svg viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>
                  </a>
                  <a href="sms:${groomPhone}" class="btn btn--outline btn--contact" title="신랑에게 문자하기">
          ...
          <div class="contact-item">
              <div class="contact-item__info">
                  <span class="contact-item__role">신부</span>
                  <div class="contact-item__name">${b.name}</div>
              </div>
              <div class="contact-item__actions">
                  <a href="tel:${bridePhone}" class="btn btn--outline btn--contact" title="신부에게 전화하기">
                      <svg viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>
                  </a>
                  <a href="sms:${bridePhone}" class="btn btn--outline btn--contact" title="신부에게 문자하기">
                      <svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 9h12v2H6V9zm8 5H6v-2h8v2zm4-6H6V6h12v2z"/></svg>
                  </a>
              </div>
          </div>
    ...
    // 부모님 성함 및 소개 영역('#greetingParents') 바로 뒤에 붙여줍니다.
