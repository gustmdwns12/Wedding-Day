...
/* ═══════════════════════════════════════════
  ...
  function initLocation() {
    ...
    // 네이버 지도 동적 로드 및 초기화
    if (w.naverMapClientId) {
      const script = document.createElement('script');
      script.type = 'text/javascript';
      script.src = `https://oapi.map.naver.com/openapi/v3/maps.js?ncpClientId=${w.naverMapClientId}`;
      
      script.onload = function () {
        // 지도 초기화 설정
        const mapOptions = {
          center: new naver.maps.LatLng(w.lat, w.lng),
          zoom: 15, // 기본 확대 정도
          zoomControl: true, // 확대/축소 버튼 노출
          zoomControlOptions: {
            position: naver.maps.Position.RIGHT_BOTTOM
          },
          scrollWheel: false // 모바일 스크롤 시 지도가 제멋대로 확대되는 것 방지
        };

        const map = new naver.maps.Map('naverMap', mapOptions);

        // 결혼식장 위치에 빨간색 기본 마커 핀 꽂기
    ...
    if (w.transit) {
      ...
      const transitContainer = $('#locationTransit');
      if (transitContainer) {
